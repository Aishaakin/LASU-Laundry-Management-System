    ...
    def handle(self, *args, **kwargs):
        ...
        for id, name, desc, price, unit, icon, features in services:
            obj, created = Service.objects.get_or_create(
                id=id,
                defaults={'name':name,'description':desc,'price':price,'price_unit':unit,'icon':icon,'features':features,'is_active':True}
            )
            self.stdout.write(f"{'Created' if created else 'Exists'}: {name}")

        # Clothing items
        items = [
            (1,'T-Shirt','tops',200,'👕'),(2,'Shirt','tops',350,'👔'),
            (3,'Blouse','tops',350,'👗'),(4,'Sweater','tops',500,'🧥'),
            (5,'Jeans','bottoms',500,'👖'),(6,'Trousers','bottoms',450,'👖'),
            (7,'Shorts','bottoms',250,'🩳'),(8,'Dress','dresses',700,'👗'),
            (9,'Gown','dresses',900,'👗'),(10,'Jacket','outerwear',800,'🧥'),
            (11,'Coat','outerwear',1200,'🥼'),(12,'Bed Sheet','others',600,'🛏️'),
            (13,'Towel','others',300,'🪥'),
        ]
        for id, name, cat, price, icon in items:
            obj, created = ClothingItem.objects.get_or_create(
                id=id,
                defaults={'name':name,'category':cat,'price':price,'icon':icon,'is_available':True}
            )
            self.stdout.write(f"{'Created' if created else 'Exists'}: {name}")
        ...
        if created:
            ...
            staff.save()
            self.stdout.write('Created staff account: staff@lasuviva.com')
